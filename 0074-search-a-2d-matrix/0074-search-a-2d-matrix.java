class Solution {
    ...
    public boolean searchMatrix(int[][] matrix, int target) {
        ...
        if (rowIdx != -1) return binarySearchOverRow(rowIdx, matrix, target); else return false;
    }
    ...
    private boolean binarySearchOverRow(int rowIdx, int[][] matrix, int target) {
        ...
        while (low <= high) {
            ...
            if (matrix[rowIdx][mid] == target) return true; else if (matrix[rowIdx][mid] > target) high = mid - 1; else low = mid + 1;
        }
        return false;
    }
}
